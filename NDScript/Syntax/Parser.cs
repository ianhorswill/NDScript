...
namespace NDScript.Syntax
{
    public static class Parser
    {
        ...
        #region Utilities
        /// <summary>
        /// Get the current line number without advancing the input.
        /// </summary>
        public static Parser<int> LineNumber = input => Result.Success(input.Line, input);

        /// <summary>
        /// Makes a version of the parser that immediately throws an exception if it fails.
        /// </summary>
        ...
        public static readonly Parser<Expression> StringLiteral =
            (from line in LineNumber
                ...
                select new Constant(line, str)).Named("string").Token();
        ...
        public static readonly Parser<Expression> Number =
            (from line in LineNumber
             ...
             select int.TryParse(text, out var n)? new Constant(line, sign * n) : new Constant(line, sign*float.Parse(text))
            ).Named("number");

        private static readonly Parser<Expression> True = Parse.String("true").Return(new Constant(0, true));
        private static readonly Parser<Expression> False = Parse.String("false").Return(new Constant(0, false));
        private static readonly Parser<Expression> Null = Parse.String("null").Return(new Constant(0, null)).Named("null");
        ...
        public static readonly Parser<Expression> VariableReference =
            ...
            select new VariableReference(line, name)).Named("variable reference");
        
        public static readonly Parser<Expression> FailExpression = 
            from line in LineNumber
            from _ in Parse.String("fail").Token() 
            select new Fail(line);
        ...
        private static readonly Parser<Func<Expression, Expression>> FunctionCall =
            ...
            select (Func<Expression,Expression>)(e => new FunctionCall(line, e, arglist.ToArray()));
        ...
        private static readonly Parser<Func<Expression, Expression>> ArrayReference =
            ...
            select (Func<Expression,Expression>)(e =>
            {
                ...
                {
                    1 => new ArrayReference(line, e, i[0]),
                    2 => new GridReference(line, e, i[0], i[1]),
        ...
        public static readonly Parser<Expression> Negation =
            ...
            select new UnaryOperatorExpression(line, UnaryOperatorExpression.Negate, v);

        public static readonly Parser<Expression> NotExpression =
            from line in LineNumber
            from _ in Parse.Char('!').Token()
            from v in PosfixExpression
            select new UnaryOperatorExpression(line, UnaryOperatorExpression.Not, v);
        ...
        public static readonly Parser<Expression> AdditiveExpression =
            from line in LineNumber
            from result in ChainOperatorCommitted(Plus.Or(Minus), MultiplicativeExpression.Named("expression"),
                (op, left, right) => new BinaryOperatorExpression(line, op, left, right))
            select result;
        #endregion
        ...
        private static readonly Parser<Func<object?, object?, State, object?>> RelationalOperator =
            ...
            .Or((Parse.Char('>').Return(BinaryOperatorExpression.Gt))).Token().Named("relational operator");
        ...
        public static readonly Parser<Expression> RelationalExpression =
            from line in LineNumber
            from result in AdditiveExpression.Then(left => RelationExpressionCompletion(left, RelationalOperator,
                    AdditiveExpression.Named("expression"),
                    (l, op, r) => new BinaryOperatorExpression(line, op, l, r))
        ...
        public static readonly Parser<Expression> EqualityExpression =
            from line in LineNumber
            from result in RelationalExpression.Then(left => RelationExpressionCompletion(left, EqualityOperator,
                    RelationalExpression.Named("expression"),
                    (l, op, r) => new BinaryOperatorExpression(line, op, l, r))
                .Or(Parse.Return(left)))
            select result;
        #endregion
        ...
        private static readonly Parser<Expression> LogicalAndExpression =
            from line in LineNumber
            from result in ChainOperatorCommitted(Parse.String("&&").Token(), EqualityExpression.Named("expression"),
                (_, left, right) => new ShortCircuitOperatorExpression(line, false, left, right))
            select result;

        private static readonly Parser<Expression> LogicalOrExpression =
            from line in LineNumber
            from result in ChainOperatorCommitted(Parse.String("||").Token(), LogicalAndExpression.Named("expression"),
                (_, left, right) => new ShortCircuitOperatorExpression(line, true, left, right))
            select result;
        #endregion
        ...
        public static readonly Parser<Expression> AssignmentExpression =
            (from line in LineNumber
                ...
                select new AssignmentExpression(line, SettableExpression.CheckSettable(lvalue), value)).Token().Named("assignment expression");
        ...
        private static readonly Parser<Statement> VariableDeclaration =
            (from line in LineNumber
                ...
                select new VariableDeclaration(line, name, value)).Token().Named("variable declaration");
        ...
        private static readonly Parser<Statement> ReturnStatement =
            (from line in LineNumber
                ...
                select new Return(line, value)).Token().Named("return statement");
        ...
        public static readonly Parser<Block> Block =
            ...
            select new Block(line, statements.ToArray())).Named("block");
        ...
        public static readonly Parser<Statement> FunctionDefinition =
            ...
            select new FunctionDeclaration(line, name, det, args.ToArray(), b)).Named("function definition");
        ...
        public static readonly Parser<Statement> IfStatement =
            ...
            select new IfStatement(line, condition, consequent, alternative)).Named("if statement");
        ...
        public static readonly Parser<Statement> WhileStatement =
            (from line in LineNumber
                ...
                select new WhileStatement(line, condition, body)).Named("while statement");
        ...
        public static readonly Parser<Statement> ChooseStatement =
            (from line in LineNumber
                ...
                select new ChooseStatement(line,rest.Prepend(first).ToArray(), mode)).Named("choose statement");
        ...
        public static readonly Parser<Statement> ForeachStatement =
            (from line in LineNumber
                ...
                select new ForeachStatement(line, name, collection, body)).Named("foreach statement");
        #endregion
